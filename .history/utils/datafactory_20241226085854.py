...
class HorizonDataFactory:
    ...
    def __init__(self, 
                 ...
                 kernel_size: tuple = (1, 288, 16),
                 stride: tuple = (1, 288, 6416),
    ...
    def _process_attribute_data(self, attr_name, data_path, label_path):
        ...
        height_padding = (16 - (data.size(1) % 16)) % 16
        top_pad = height_padding // 2
        bottom_pad = height_padding - top_pad
        
        # Apply padding
        data = F.pad(data, [
            0, 0,  # width dimension - no padding
            top_pad, bottom_pad,  # height dimension
            0, 0   # channel dimension - no padding
        ])
        
        # Same padding for labels
        labels = F.pad(labels, [
            0, 0,
            top_pad, bottom_pad,
            0, 0
        ...
        # Modify kernel size and stride to get desired output
        # To get 16 in height dimension: kernel_size[1] and stride[1] should be adjusted
        kernel_h = data.size(1) // 16  # This will be the kernel height to get 16 segments
        stride_h = kernel_h  # Use same stride as kernel size for non-overlapping windows
        
        # Apply sliding window (unfold)
        data = data.unfold(1, kernel_h, stride_h)  # This will give us 16 segments in height
