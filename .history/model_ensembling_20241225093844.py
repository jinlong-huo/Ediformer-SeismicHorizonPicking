...
class AdvancedEnsembleLearner:
    ...
    def val_meta_models(self, classifier, val_loader, attr_name, epoch, early_stopping, save_path):
        classifier.eval()
        val_loss = 0
        correct = 0
        total = 0
    ...
    def train_ensemble(self, 
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(classifier_optimizers, 
                                                            mode='min', 
                                                            factor=0.1, 
                                                            patience=5)
        ...
        for epoch in range(epochs):
            ...
            for idx, (classifier, ((attr_name, train_loader), (attr_name, val_loader)), optimizer) in enumerate(zip(self.classifiers, zip(attribute_dataloaders, validation_dataloaders), classifier_optimizers)):
                ...
                self.val_meta_models(classifier, train_loader,   attr_name,  epoch, early_stopping, mm_path)
