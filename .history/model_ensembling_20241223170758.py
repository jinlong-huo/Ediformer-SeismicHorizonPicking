...
class AdvancedEnsembleLearner:
    ...
    def train_meta_models(self, classifier, train_loader, val_loader, optimizer, criterion, attr_name, epoch, early_stopping, save_path):
        classifier.train()
        total_loss = 0
        accuracy = []
    ...
    def val_meta_models(self, classifier, train_loader, val_loader, optimizer, criterion, attr_name, epoch, early_stopping, save_path):
        classifier.eval()
        val_loss = 0
        correct = 0
        total = 0
    ...
    def train_fusion_model(self, attribute_dataloaders, optimizer, criterion, epoch):
    ...
    def train_ensemble(self, 
        ...
        for epoch in range(epochs):
            ...
            for idx, (classifier, ((attr_name, train_loader), (attr_name, val_loader)), optimizer) in enumerate(zip(self.classifiers, zip(attribute_dataloaders, validation_dataloaders), classifier_optimizers)):
                ...
                self.train_meta_models(classifier, train_loader, val_loader, optimizer, criterion, attr_name, epoch, early_stopping, mm_path)
                
                self.val_meta_models(classifier, train_loader, val_loader, optimizer, =criterion, attr_name, epoch, early_stopping, mm_path)
                ...
                # print(f"Meta-Model {attr_name} - Epoch {epoch+1}: Training Loss: {total_loss/len(train_loader)} Training Acc: {np.array(accuracy).mean()*100}")
                
                # # Stage 1.2 meta-models Validation keep in same loop
                # classifier.eval()
                # val_loss = 0
                # correct = 0
                # total = 0
                ...
                #     for batch_x, batch_y in val_loader:
                #         batch_y = torch.squeeze(batch_y.long())
                #         outputs, _ = classifier(batch_x)   # outputs: torch.Size([16, 7, 64, 288])
                #         loss = criterion(outputs[6], batch_y.long())
                #         val_loss += loss.item()
                #         _, predicted = outputs[6].max(1)
                #         total += batch_y.size(0)
                ...
                # print(f"Meta-Model {attr_name} - Epoch {epoch + 1}: Validation Loss: {val_loss.mean() / len(val_loader):.4f}, Validation Accuracy: {100 * correct / (total * batch_x.shape[2] * batch_x.shape[3]):.4f}%")
                # # attr_name
