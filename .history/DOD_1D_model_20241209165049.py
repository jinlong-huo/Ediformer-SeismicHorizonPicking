...
class HighMixer(nn.Module):
    def __init__(self, dim, kernel_size=3, stride=1, padding=1,
        **kwargs, ):
        ...
        # self.cnn_in = cnn_in = math.floor(dim // 2)
        # self.pool_in = pool_in = math.floor(dim // 2)
    ...
    def forward(self, x):
        ...
        cx = self.conv1(cx) # up sampling
        cx = self.proj1(cx) # keep dim
        cx = self.mid_gelu1(cx)
            
        px = x[:,self.cnn_in:,:,:].contiguous() # same as cx
        
        px = self.Maxpool(px)
        px = self.proj2(px)
        px = self.mid_gelu2(px)
        
        # this cat makes the dim 1 double 
...
class LowMixer(nn.Module):
    ...
    def forward(self, x):
        ...
        # xa = self.pool(x)
        xa = x 
        xa = xa.permute(0, 2, 3, 1).view(B, -1, self.dim) # N = H * W
        B, N, C = xa.shape # 2 256 36
        qkv = self.qkv(xa).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple)
        
        # B self.num_heads, N, C   3, 2, 1, 256, 36 --> 2, 1, 256, 36
        xa = self.att_fun(q, k, v, B, N, C) # --> 2, 36, 256
        # xa = xa.view(B, C, int(N**0.5), int(N**0.5))#.permute(0, 3, 1, 2)
        xa = xa.view(B, C, -1, 1)#.permute(0, 3, 1, 2)
        
        # xa shape B, C, 16, 16
        # xa = xa.view(B, C, int(N**0.5), int(N**0.5))#.permute(0, 3, 1, 2)
        
        # we change the uppool into the cat with second dimension
        # xa = self.uppool(xa)
        
        # xa = torch.cat((xa, xa), dim=1)
        return xa
...
class Mixer(nn.Module):
    ...
    def forward(self, x):
        ...
        # hx3 shape 2 72 256 1   hx4 2 36 512 1   2 36 256 1  
        # lx3 shape 2 72 256 1    lx4 2 36 512 1  2 18 256 1 
...
class DexiNed(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=False, attn_drop=0., drop_path=0.,
        ...
        self.dblock_5 = _DenseBlock(3, 512, 256)
        self.dblock_6 = _DenseBlock(3, 256, 256)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)  # 3
        self.maxpool2 = nn.MaxPool2d(kernel_size=3, stride=1, padding=1)  # 3
        ...
        self.side_4 = SingleConvBlock(512, 256, 1)
        self.side_5 = SingleConvBlock(256, 256, 1)
        ...
        self.attn1 = attn(dim[0], num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, attention_head=attention_head, pool_size=pool_size,)
        
        self.attn2 = attn(dim[1], num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, attention_head=attention_head, pool_size=pool_size,)
        
        self.attn3 = attn(dim[2], num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, attention_head=attention_head, pool_size=pool_size,)
        
        self.attn4 = attn(dim[3], num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, attention_head=attention_head, pool_size=pool_size,)
    ...
    def forward(self, x): 
        ...
        # block_5_add = self.maxpool(block_5_add)
        block_6, _ = self.dblock_6([block_5_add, block_6_attn_dense])
        # block_6 = block_5_add + block_6_attn_dense
