...
#     # device = "cpu"
#     input = torch.rand(batch_size, 1, 60, 288).to(device)             #这里是batch_size的设置 C H W 1 1 288
...
def main(args):
    ...
    if not args.is_testing:
        ...
        Label = Label.contiguous().view(-1, kc, kh, kw)
        Label = torch.tensor(Label, dtype=torch.float)
        print(Data.shape, Label.shape) # torch.Size([1830, 1, 288, 64]) torch.Size([1769, 1, 288, 64])
