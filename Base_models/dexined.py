...
#    outputs, ensemble = model(input_tensor)
#    ```
#
# Output
# - List of 7 tensors: Six edge maps at different scales plus one fused output
# - Ensemble result: Final edge prediction with shape [batch_size, 1, height, width]
# 
# Architecture combines dense blocks with skip connections for multi-scale edge detection,
# adapted for seismic horizon picking with 7-class output in the fused map.
...
class _DenseLayer(nn.Sequential):
    def __init__(self, input_features, out_features):
        ...
        self.add_module('conv1', nn.Conv2d(input_features, out_features,
                                           kernel_size=(3,3), stride=1*1, padding=(2), bias=True)),
...
class DexiNed(nn.Module):
    ...
    def __init__(self):
        ...
        self.block_1 = DoubleConvBlock(1, 32, 64, stride=2, use_act=True)
        self.block_2 = DoubleConvBlock(64, 128, stride=1, use_act=True)
        ...
        self.block_cat = SingleConvBlock(6, 7, stride=1, use_bs=True)
        self.block_cat2 = SingleConvBlock(7, 1, stride=1, use_bs=True)
    ...
    def forward(self, x):
        ...
        # print(out_1.shape, out_6.shape)
        block_cat = torch.cat(results, dim=1)

        block_cat = self.block_cat(block_cat)
        result_ensemble = self.block_cat2(block_cat)
        results.append(block_cat)

        return results, result_ensemble
